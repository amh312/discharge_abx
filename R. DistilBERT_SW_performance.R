#MODEL PERFORMANCE (chunked + max-probability aggregation model)
...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_calibration_curve_chunked.csv")
...
###CSV for manual checking of most-truncated discharge letters
disc_token_no |>
  filter(truncated == TRUE) |>
  arrange(desc(num_tokens)) |>
  head(30) |>
...
d_roc <- roc_maker(
  ...
  perf_df %>% select(prob),
  "Overall model (sliding window)",
  "dischargeab_roc_chunked"
)
ggsave(
  filename = "dischargeab_roc_chunked.png",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "dischargeab_roc_chunked.pdf",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "dischargeab_calib_chunked.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  dpi = 300
...
title(
  main = paste0(
    "Overall model (sliding window) PR curve\n(AUC = ",
    round(auprc, 2),
    ")"
  )
)
...
pdf("dischargeab_pr_chunked.pdf", width = 6, height = 6)
plot(
  prc$curve[, 1],
  prc$curve[, 2],
  type = "n",
...
time_df1[1, ] <- c("lang_performance_chunked.R", time_taken)
