...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_access_calibration_curve_fullbert.csv")
...
##Read-in
perf_df <- read_csv("access_fullbert_preds.csv")
perf_ci2 <- read_csv("performance_metrics_fullbert.csv")
...
d_roc <- roc_maker(
  ...
  perf_df %>% select(prob),
  "Access model (BERT-base)",
  "fullbert_accessab_roc"
)

ggsave(
  filename = "fullbert_accessab_roc.png",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "fullbert_accessab_roc.pdf",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "fullbert_accessab_calib.png",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "fullbert_accessab_calib.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
png("fullbert_accessab_pr.png", width = 6, height = 6, units = "in", res = 300)
...
title(
  main = paste0(
    "Access model (BERT-base) PR curve\n(AUC = ",
    round(auprc, 2),
    ")"
  )
)
...
pdf("fullbert_accessab_pr.pdf", width = 6, height = 6)
...
write_csv(prc_df, "sourcedata_access_prc_fullbert.csv")
...
write_csv(perf_cis, "access_performance_metrics_fullbert.csv")
...
write_csv(truncated_perf_cis, "ac_truncated_performance_metrics_fullbert.csv")
...
time_df1[1, ] <- c("lang_full_access_performance.R", time_taken)
