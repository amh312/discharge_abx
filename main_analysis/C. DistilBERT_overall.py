...
def remove_testpatients(train_df, test_df,key_df):
    ...
    #keep record of those removed from training set
    ...
    #filter discharge letters out of training set based on testing set
    train_disc_bertdf2 = train_df.to_pandas()
    ...
    #return both the filtered training set and the texts of those removed from the training set
...
def bert_predict(mod):
    ...
    # create dataframe with predictions, probabilities, and labels
...
time_df1 = pd.DataFrame({"Script": ["C. DistilBERT_overall.py"], "Time (secs)": [time_taken]})
