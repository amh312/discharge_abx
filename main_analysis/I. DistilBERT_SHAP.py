...
for j in range(6):
    ...
    for i in range(len(shapvalues)):
        ...
        filename2 = f"shap_text_plot_{filename}_{i+1}.html"
        with open(filename2, "w") as f:
            f.write(html_str)

    ###Extract all SHAP values across questionnaires to df for bar charts
    disc_shapdf1 = shap_extractor(shapvalues)
    disc_shapdf1.to_csv(f'shaptokens_df_{j}.csv', index=False)

##Fictional discharge summaries for manuscript text plot figures

###Inappropriate discharge antibiotic
madeup_shap("Dear Mr ___, it was a pleasure looking after you during your hospital stay. You were admitted with cough, chest pain, and shortness of breath after your recent knee surgery. You were diagnosed with an irregular heart beat and a heart attack. You underwent a procedure to unblock your heart arteries and were given blood thinners and antibiotics. We have temporarily held your atorvastatin, which you should restart in 1 week. You should attend your orthopaedic appointment in 2 weeks and your cardiology appointment in 1 month. Please contact your GP if you develop any new symptoms or have any concerns. We wish you a speedy recovery.", "shap_text_plot_madeup.html")
...
time_df1 = pd.DataFrame({"Script": ["I. DistilBERT_SHAP.py"], "Time (secs)": [time_taken]})
