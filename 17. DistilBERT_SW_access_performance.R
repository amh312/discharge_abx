#MODEL PERFORMANCE (ACCESS PREDICTION, chunked + max-probability aggregation model)
...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_access_calibration_curve_chunked.csv")
...
d_roc <- roc_maker(
  ...
  perf_df %>% select(prob),
  "Access model (sliding window)",
  "accessab_roc_chunked"
)

ggsave(
  filename = "accessab_roc_chunked.png",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "accessab_roc_chunked.pdf",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "accessab_calib_chunked.png",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "accessab_calib_chunked.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
png("accessab_pr_chunked.png", width = 6, height = 6, units = "in", res = 300)
...
title(
  main = paste0(
    "Access model (sliding window) PR curve\n(AUC = ",
    round(auprc, 2),
    ")"
  )
)
...
pdf("accessab_pr_chunked.pdf", width = 6, height = 6)
...
time_df1[1, ] <- c("lang_access_performance_chunked.R", time_taken)
