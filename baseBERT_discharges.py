...
def remove_testpatients(train_df, test_df,key_df):
    ...
    #get texts from train and test sets
...
discdf_token_no.to_csv("disc_token_no_fullbert.csv", index=False)
...
train_ref.to_csv("train_ref_fullbert.csv", index=False)
train_removed.to_csv("train_removed_fullbert.csv", index=False)
...
###Load-in pre-trained BERT model and assign to MPS or CPU
discmodel = BertForSequenceClassification.from_pretrained("bert-base-uncased", num_labels=2)
discmodel.to(device)
...
preds_perf_df.to_csv("fullbert_preds.csv", index=False)

##Save model and tokeniser

savdirec = "./pt_disc_fullbert"
...
time_df1 = pd.DataFrame({"Script": ["BERT_full_discharges.py"], "Time (secs)": [time_taken]})
...
discmodel = BertForSequenceClassification.from_pretrained("./pt_disc_fullbert", num_labels=2)
discmodel.to(device)
sample_row = (
    test_disc_bertdf
...
time_df2 = pd.DataFrame({"Script": ["BERT-base single prediction"], "Time (secs)": [time_taken.total_seconds()]})
time_df2.to_csv("fullbert_overall_single_prediction.csv", index=False)
