...
To test the functionality of the code for the main analysis without requiring download of the real PhysioNet datasets, download all csv files in this repository, and **remove the "_test" suffix from any csv file names where it is present**. Once the packages in *packages.txt* are installed, all scripts apart from those that are specifically tailored to the engineering and descriptive characteristics of the PhysioNet datasets (*lang_disc_clearning.R* and *lang_descriptive.R*) can then be run in the following order: 
