#Longformer discharge antibiotic prediction (Access model)

##Packages

from transformers import LongformerTokenizerFast, LongformerForSequenceClassification, set_seed
import torch
...
###Max sequence length - see Longformer_discharges.py for rationale (1024
###covers ~99% of texts by token count; padding to the full 4096 Longformer
###supports was found far too slow/memory-heavy for this dataset on MPS).
MAX_LENGTH = 1024
...
###Tokenise dataset
tokeniser = LongformerTokenizerFast.from_pretrained('allenai/longformer-base-4096')
...
train_ref.to_csv("ac_train_ref_longformer.csv", index=False)
train_removed.to_csv("ac_train_removed_longformer.csv", index=False)
...
###Load-in pre-trained Longformer-base model and assign to MPS or CPU
discmodel = LongformerForSequenceClassification.from_pretrained("allenai/longformer-base-4096", num_labels=2)
discmodel.to(device)
...
preds_perf_df.to_csv("access_longformer_preds.csv", index=False)

##Save model and tokeniser

savdirec = "./pt_access_disc_longformer"
...
time_df1 = pd.DataFrame({"Script": ["Longformer_access.py"], "Time (secs)": [time_taken]})
...
discmodel = LongformerForSequenceClassification.from_pretrained("./pt_access_disc_longformer", num_labels=2)
discmodel.to(device)
sample_row = (
    test_disc_bertdf
...
time_df2 = pd.DataFrame({"Script": ["Access Longformer single prediction"], "Time (secs)": [time_taken.total_seconds()]})
time_df2.to_csv("access_longformer_single_prediction.csv", index=False)
