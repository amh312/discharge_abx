...
def bert_predict(mod):
    ...
    # create a df with predictions, probabilities, and labels
...
train_ref.to_csv("ac_train_ref_fullbert.csv", index=False)
train_removed.to_csv("ac_train_removed_fullbert.csv", index=False)
...
time_df1 = pd.DataFrame({"Script": ["T. baseBERT_Access.py"], "Time (secs)": [time_taken]})
...
discmodel = BertForSequenceClassification.from_pretrained("./pt_access_disc_fullbert", num_labels=2)
discmodel.to(device)
sample_row = (
    test_disc_bertdf
...
time_df2 = pd.DataFrame({"Script": ["Access BERT-base single prediction"], "Time (secs)": [time_taken.total_seconds()]})
time_df2.to_csv("access_fullbert_single_prediction.csv", index=False)
