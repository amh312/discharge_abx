...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_bow_access_calibration_curve.csv")
...
ggsave(
  filename = "bow_accessab_calib.png",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "bow_accessab_calib.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  units = "in",
...
png("bow_accessab_pr.png", width = 6, height = 6, units = "in", res = 300)
...
pdf("bow_accessab_pr.pdf", width = 6, height = 6)
...
perf_cis <- perf_cis %>%
  mutate(
    `Access BOW antibiotic model (95% CI)` = glue(
...
time_df1[1, ] <- c("BOW_access_performance.R", time_taken)
