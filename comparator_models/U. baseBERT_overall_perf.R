...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_fullbert_calibration_curve.csv")
...
###Check proportion of truncations
...
d_roc <- roc_maker(
  ...
  perf_df %>% select(prob),
  "Overall model (BERT-base)",
  "fullbert_dischargeab_roc"
)
ggsave(
  filename = "fullbert_dischargeab_roc.png",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "fullbert_dischargeab_roc.pdf",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "fullbert_dischargeab_calib.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  dpi = 300
...
title(
  main = paste0(
    "Overall model (BERT-base) PR curve\n(AUC = ",
    round(auprc, 2),
    ")"
  )
)
...
pdf("fullbert_dischargeab_pr.pdf", width = 6, height = 6)
plot(
  prc$curve[, 1],
  prc$curve[, 2],
  type = "n",
...
time_df1[1, ] <- c("U. baseBERT_overall_perf.R", time_taken)
