...
calibmaker <- function(actc, predp, outc) {
  ...
  #save sourcedata
  write_csv(ur_calib_df, "sourcedata_bow_calibration_curve.csv")
...
ggsave(
  filename = "bow_dischargeab_roc.png",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "bow_dischargeab_roc.pdf",
  plot = d_roc,
  width = 10,
  height = 10,
  units = "in",
...
ggsave(
  filename = "bow_dischargeab_calib.pdf",
  plot = d_calib,
  width = 10,
  height = 10,
  dpi = 300
...
png("bow_dischargeab_pr.png", width = 6, height = 6, units = "in", res = 300)
plot(
  prc$curve[, 1],
  prc$curve[, 2],
  type = "n",
...
title(
  main = paste0("Overall BOW model PR curve\n(AUC = ", round(auprc, 2), ")")
)
dev.off()

pdf("bow_dischargeab_pr.pdf", width = 6, height = 6)
plot(
  prc$curve[, 1],
  prc$curve[, 2],
  type = "n",
...
perf_cis <- perf_cis %>%
  mutate(
    `Overall BOW antibiotic model (95% CI)` = glue(
...
time_df1[1, ] <- c("X. BOW_overall_perf.R", time_taken)
