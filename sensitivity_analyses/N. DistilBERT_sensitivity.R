...
#This R script takes the prediction/label outputs of L. DistilBERT_stability.py and M. DistilBERT_timesens.py and
#effectively repeats the performance evaluation done for the two models in scripts E and F, but for the stability and
#time sensitivity analyses.

#It also performs a fairness analysis for the two models, which does not require model
#retraining but rather subsets the csvs outputted by scripts C and D based on protected characteristics and
# assesses model performance demographic subgroup by subgroup.
# Given the larger amount of performance data generated by the fairness analyses,
#it is presented instead as dot plots with error bars for 95% confidence intervals. The script also includes a
#threshold adjustment analysis where decision thresholds are varied and the effect on model performance for each
#demographic group is plotted on dot plots.
